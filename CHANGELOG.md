...
- **Cutoff tuning v2: Morphology-sensitive cap compression** - Enhanced Spiky mode with graduated cap reductions based on morphology score. High-morphology environments (tight spaces, poor visibility, low connectivity) now compress severity caps more aggressively, increasing cutoff frequency to target ranges: 5-10% for confined/derelict, 2-5% for open. Implementation uses two-threshold system (morph >= 0.9: -1 cap, morph >= 1.4: -2 cap) to treat structural fragility as a cutoff multiplier while preserving Normal/Calm behavior. Note: preset names updated to setting-neutral terminology in v1.0.
- **Adaptive weighting validation** - Validated adaptive weighting system effectiveness across all presets/modes. Enhanced penalty curve (v0.1 -> v0.2) with tiered recency penalties achieves target variety: confined/derelict max 14.5% (exceeds 15% stretch goal), open 27.5% (within structural content limit). Comprehensive test suite added with severity band analysis tooling to diagnose content gaps vs implementation issues. Note: preset names updated to setting-neutral terminology in v1.0.
- **Aftermath phase content expansion** - Added 10 minimal aftermath entries to address phase coverage gaps. Open aftermath expanded from 3 to 6 events with non-attrition diversity (information, visibility, terrain tags). Sea aftermath added 2 dedicated entries. Populated, derelict, confined received 1-2 aftermath entries each. All entries use low severity bands (1-6) appropriate for aftermath consequences. Enables Presets × (Approach/Engage/Aftermath) × Normal suite completion at batch size 200. Note: preset names updated to setting-neutral terminology in v1.0.
